  ...
  return (
    <article className={`prod-card ${isDepleted ? 'card-out' : ''}`}>
      <Link to={`/product/${productSlug}`} className="prod-img">
        {product.image_file ? (
          <img src={`/${product.image_file}`} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
      ...
      {!isDepleted && (
        <button className="quick-add" onClick={(e) => { e.preventDefault(); addToCart(product, 1); }} aria-label={`Add ${product.name} to cart`}>+</button>
      )}
